...
unemp_rate <- get_acs(geography = "tract", variables = "B23025_005", state = 37,
                      key = api_key, summary_var = "B23025_001", geometry = T)

cleaned <- unemp_rate %>%
  mutate(acs_unemp_rate = estimate / summary_est) %>%
  mutate(acs_unemp_rate = ifelse(is.nan(acs_unemp_rate), 0, acs_unemp_rate)) %>%
  mutate(acs_unemp_rate = round(acs_unemp_rate, digits = 2)) %>%
  rename(acs_unemp_total = estimate) %>%
  mutate(county_geoid = substr(GEOID, 1, 5)) %>%
  clean_names() %>%
  select(geoid, name, county_geoid, acs_unemp_total, acs_unemp_rate) %>%
  left_join(nc_counties_merge, by = "county_geoid")

natural_breaks <- classIntervals(filter(cleaned, !(is.na(acs_unemp_rate)))$acs_unemp_rate, 5, style = "jenks")

final <- cleaned %>%
  mutate(natural_breaks = cut(acs_unemp_rate, breaks = c(-0.01, natural_breaks$brks[2:length(natural_breaks$brks)]))) %>%
...
p1 <- final %>%
  ...
  labs(title = "Unemployment Rate, Population Over 16 Years",
       subtitle = "Unemployment Rate by Census Tract for the population 16 years and over\nCensus Tracts with no or insufficient data have been removed",
       caption = "<b>Source:</b> Census Bureau 5 Year ACS") +
  map_theme

p1 + inset_element(logo, left = 0.7, bottom = 0, right = 1, top = 0.09, align_to = 'full')

ggsave(filename = "acs-unemp.png", device = "png",
       path = here("plots/"), dpi = "retina", width = 16, height = 9)

final_dat <- final %>%
  mutate(acs_unemp_score = (acs_unemp_rate - mean(final$acs_unemp_rate, na.rm = T)) / sd(final$acs_unemp_rate, na.rm = T)) %>%
  select(geoid, acs_unemp_score) %>%
  as_tibble() %>%
  select(-geometry) %>%
  right_join(final) %>%
  as_tibble() %>%
  rename(tract_name = name, tract_geoid = geoid) %>%
  select(tract_geoid, tract_name, county_geoid, county_name, county_full_name, acs_unemp_total, acs_unemp_rate, acs_unemp_score)

write_csv(final_dat, here("composite/acs-unemp.csv"))
