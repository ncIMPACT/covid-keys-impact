...
cleaned <- monthly_sales %>%
  filter(county != "Foreign") %>%
  filter(year == 2019 | year == 2020) %>%
  filter(month(date) <= month(max(date))) %>%
  group_by(county, year) %>%
  summarise(taxable_sales = sum(taxable_sales, na.rm = T)) %>%
  ungroup() %>%
  pivot_wider(names_from = year, values_from = taxable_sales) %>%
...
p1 <- final %>%
  ...
  labs(title = "Total Taxable Sales Change",
       subtitle = "Percent change in total taxable sales during the same period (Jan to Sep) for 2019 and 2020",
       caption = "<b>Source:</b> NC Department of Revenue") +
  map_theme

p1 + inset_element(logo, left = 0.7, bottom = 0, right = 1, top = 0.09, align_to = 'full')

ggsave(filename = "taxable-sales.png", device = "png",
       path = here("plots/"), dpi = "retina", width = 16, height = 9)         

final_dat <- final %>%
  as_tibble() %>%
  replace_na(list(change = 0)) %>%
  mutate(tax_sales_score = ((change - mean(final$change, na.rm = T)) / sd(final$change, na.rm = T)) * -1) %>%
  rename(tax_sales_change = change, name = county) %>%
  select(name, namelsad, geoid, tax_sales_change, tax_sales_score)

write_csv(final_dat, here("composite/tax-sales.csv"))
