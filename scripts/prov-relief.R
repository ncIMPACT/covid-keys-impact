...
prov_relief <- read_csv(here("data/provider-relief-fund.csv"))
...
cleaned <- prov_relief %>%
  ...
  mutate(across(.cols = starts_with("payment"), .fns = ~str_remove_all(.x, "\\$"))) %>%
  mutate(across(.cols = starts_with("payment"), .fns = ~str_remove_all(.x, "\\,"))) %>%
  mutate(across(.cols = starts_with("payment"), .fns = as.numeric)) %>%
  mutate(total_paid = payment) %>%
  group_by(city, state) %>%
...
p1 <- final %>%
  ...
  labs(title = "HHS Provider Relief Fund\nDistributions Per Capita",
       subtitle = "Per capita reimbursement from HHS Provider Relief Fund",
       caption = "<b>Source:</b> HHS TAGGS") +
  map_theme

p1 + inset_element(logo, left = 0.7, bottom = 0, right = 1, top = 0.09, align_to = 'full')

ggsave(filename = "prov-relief.png", device = "png",
